...
router.get('/', function(req, res, next) {
    if (req.query.id) {
      var query = "SELECT comptes.ID_Compte, CONCAT(u.Labo, '_', NumeroCompte, '_', " +
          ...
          "FROM comptes JOIN (SELECT ID_Compte, ID_User FROM utilcomptes WHERE ID_User = '" + req.query.id + "') uc " +
          "ON uc.ID_Compte = comptes.ID_Compte JOIN (SELECT ID_User, Labo, projectnum FROM utilisateur) u " +
